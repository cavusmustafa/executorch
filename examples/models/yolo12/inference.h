...
std::vector<Detection> infer_yolo_once(
  ...
  auto total_time = std::chrono::duration_cast<std::chrono::microseconds>(t4 - t1);
  auto before_time = std::chrono::duration_cast<std::chrono::microseconds>(t2 - t1);
  auto after_time = std::chrono::duration_cast<std::chrono::microseconds>(t4 - t3);
  //auto scale_time = std::chrono::duration_cast<std::chrono::microseconds>(t1_2 - t1_1);
  auto b1_time = std::chrono::duration_cast<std::chrono::microseconds>(t1_4 - t1_3);
  std::cout << "DEBUG - timing - infer: " << infer_time.count() << ", total: " << total_time.count() << ", before: " << before_time.count() << ", after: " << after_time.count() << ", b1: " << b1_time.count() << std::endl;
  return detections;
}
#endif // INFERENCE_H
