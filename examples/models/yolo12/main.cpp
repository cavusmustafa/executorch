...
int main(int argc, char** argv) {
  ...
  //  int pad_x, pad_y;
  //  float scale;
  //  cv::Mat scaled_input = scale_with_padding(frame, &pad_x, &pad_y, &scale, img_dims);
  //  std::shared_ptr<executorch::aten::Tensor> prepared_input = prepare_input(scaled_input, img_dims);
  //  executorch::aten::Tensor output_tensor = execute_frame(yolo_module, prepared_input);
  //  std::vector<Detection> output = process_output(output_tensor, DEFAULT_YOLO_CONFIG, pad_x, pad_y, scale);
  ...
  while (true) {
    ...
    //std::cout << "DEBUG - main-loop - B - ready: " << ready_q.size() << ", scale: " << scale_q.size() << ", input: " << input_q.size() << ", excecute: " << execute_q.size() << ", output: " << output_q.size() << std::endl;
    //std::vector<Detection> output =
    //    infer_yolo_once(yolo_module, frame, img_dims, DEFAULT_YOLO_CONFIG);
    ...
    while (!ready_q.empty() && scale_q.size() < 2) {
    ...
    while (!scale_q.empty() && input_q.size() < 2) {
      //auto pair_s = std::move(scale_q.front());
      auto status = scale_q.front().second.wait_for(std::chrono::milliseconds(1));
      //pair_s.second.wait();
      //auto status = std::future_status::ready;
      if (status == std::future_status::ready) {
          cv::Mat scaled_input = scale_q.front().second.get();
          input_q.push(std::make_pair(scale_q.front().first, std::async(std::launch::async, prepare_input, scaled_input, img_dims)));
          scale_q.pop();
      } else {
          break;
      }
    }
    while (!input_q.empty() && execute_q.size() < 2) {
      //auto pair_i = std::move(input_q.front());
    ...
    while (!execute_q.empty() && output_q.size() < 2) {
      //auto pair_e = std::move(execute_q.front());
    ...
    while (!output_q.empty()) {
      //auto pair_o = std::move(output_q.front());
